...
function handleUpdateHotel($pdo, $postData)
{
    ...
    if (empty($id) || empty($hotels['name']) || empty($hotels['park']) || !is_array($seasons)) {
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'Invalid input data. Ensure "id", "name", "park", and valid "seasons" are provided.']);
        return;
    }
    ...
    try {
        ...
        foreach ($seasons as $season) {
            ...
            if (empty($rate_id) || empty($start_date) || empty($end_date) || $rate === null) {
                throw new Exception("Invalid season data provided for rate_id: " . ($rate_id ?? 'unknown'));
            }
